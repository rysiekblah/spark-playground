 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of list with one element") {
    assert(sum(List(34)) === 34)
  }

  test("sum of negative nd positive values") {
    assert(sum(List(-1, 1)) === 0)
    assert(sum(List(-1, 2, -1, 3, -2)) === 1)
  }
  ...
  test("max od negaive and positive values") {
    assert(max(List(-1, 1)) === 1)
    assert(max(List(1, -1)) === 1)
    assert(max(List(0, -1)) === 0)
